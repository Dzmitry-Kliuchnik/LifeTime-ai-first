...
export function useResponsiveGrid(options: ResponsiveOptions) {
  ...
  const fitsInViewport = computed(() => 
    gridWidth.value <= windowWidth.value - 40
  )
  ...
  const calculateOptimalColumns = (targetCellSize: number = options.baseConfig.cellSize): number => {
...
export function useLifetimeGridResponsive(baseCellSize: number = 12) {
  ...
  const yearsPerRow = computed(() => 
    lifetimeGridConfig.value.columns / 52
  )
