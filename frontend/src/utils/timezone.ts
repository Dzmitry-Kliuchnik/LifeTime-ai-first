...
export function getTimezoneInfo(timezone: string, date: Date = new Date()): TimezoneInfo {
  ...
  try {
    ...
    throw new TimezoneError(`Failed to get timezone info: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
export function convertToTimezone(date: Date, timezone: string): Date {
  ...
  try {
    ...
    throw new TimezoneError(`Failed to convert to timezone: ${error instanceof Error ? error.message : String(error)}`)
  }
}
...
export function calculateWeeksSinceBirthInTimezone(
  ...
  const targetDate = referenceDate ? convertToTimezone(referenceDate, timezone) : getCurrentDateInTimezone(timezone)
 ...
 */
export function getCurrentWeekInfoInTimezone(
  dateOfBirth: Date, 
  timezone: string
) {
  validateTimezone(timezone)
 ...
 */
export function getWeekSummaryInTimezone(
  dateOfBirth: Date, 
  weekIndex: number, 
  timezone: string
) {
